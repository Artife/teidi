...
$pagina = 'Lista de reglas desactivadas';
$migas = array('');
$migas_url = array('');
...
?>
	...
	#vinculo_reactivar_reglas, #vinculo_eliminar_reglas {
		display:none; 
	}
	</style>
</head>
                        ...
                        <div class="ibox-title">
                            <div class="ibox-tools">
								...
								<ul class="dropdown-menu dropdown-user">									
									<li><a id="vinculo_reactivar_reglas" href="#"> <i class="fa fa-low-vision text-navy"></i> Reactivar Reglas</a></li>
									<li><a id="vinculo_eliminar_reglas" href="#"> <i class="fa fa-low-vision text-navy"></i> Eliminar Reglas</a></li>
								</ul>
                            </div>
                        </div>
                        <div class="ibox-content">
                            <div class="row">
								...
								<input id="accion" name="accion" type="hidden" value="" required>
									<div class="col-sm-2 m-b-xs"><h5>Grupo de alimentos: </h5> 										
										<select class="input-sm form-control input-s-sm" style="padding-top:0px;" id="supergrupo" name="supergrupo">										
											<option>Todos</option>
											<?php echo gx_consultar_supergrupos(''); ?>																			
										</select>
									</div>
									<div class="col-sm-2 m-b-xs"><h5>Frecuencia: </h5> 									
										<select class="input-sm form-control input-s-sm" style="padding-top:0px;" id="frecuencia" name="frecuencia">										
											<option>Todos</option>
							...
							<form id="formulario_reglas" action="#" method="post">
                                ...
                                <table  id="example" class="table table-striped dataTables-example">
                                    ...
                                    <tr>
                                        ...
                                        <th class="marcar_todos">Grupo de alimentos</th>
										<th>Frecuencia </th>
                                        <th>Unid. mínimas </th>
                                        <th>Unid. máximas</th>
                                    </tr>
	...
	<script>
        ...
        $(document).ready(function(){
			var table_buscar = $('#example').DataTable({  
				...
				processing: true,
				erverSide: true,							
				ajax: "<?php echo $url_app; ?>crear-archivo-txt-reglas-desactivadas.php",
				deferRender:    true,
				scrollCollapse: true,
				scroller:       false,
				serialize:		true,
			...
			//Filtros de la tabla	
			$('#supergrupo').on( 'keyup change', function () {
			...
			$('#frecuencia').on( 'keyup change', function () {
				var cur_value = $('option:selected',this).text();
				if(cur_value == 'Todos'){
					cur_value = '';
				}
		...
		$(".marcar_todos").on("click", function () {
			...
			if(total_marcados == 0){
				$('#vinculo_reactivar_reglas').css('display', 'none');
				$('#vinculo_eliminar_reglas').css('display', 'none');
			}
			if(total_marcados >= 1){
				$('#vinculo_reactivar_reglas').css('display', 'block');
				$('#vinculo_eliminar_reglas').css('display', 'block');
			}
		});  
		$('#example').on( 'click', 'tr', function () {
			var total_marcados = $('.marcar').filter(':checked').length
		...
		$("#vinculo_reactivar_reglas").on("click", function () {
            $('#formulario_reglas').attr('action', "<?php echo $url_app; ?>reactivar-reglas");
			$('#formulario_reglas').submit();
        });
		$("#vinculo_eliminar_reglas").on("click", function () {
            $('#formulario_reglas').attr('action', "<?php echo $url_app; ?>eliminar-reglas");
			$('#formulario_reglas').submit();
        });
	</script>
	<?php $conn->close(); ?>

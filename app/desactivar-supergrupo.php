...
foreach($_POST as $variable => $id_usuario)  {	
	...
	if(!empty($id_usuario)){		
		...
		if(is_numeric($variable)) { 			
			gx_desactivar_supergrupos($variable);
			$i++;
		}
	}
}
...
if($i > 1){
	$_SESSION['mensaje'] = 'supergrupo_desactivados';	
}else{
	$_SESSION['mensaje'] = 'supergrupo_desactivado';		
}
header('location:'.$url_app.'lista-super-grupo');

...
$pdf_introduccion 					= $_POST['pdf_introduccion'];
$pdf_equivalencia_medidas_caceras 	= $_POST['pdf_equivalencia_medidas_caceras'];
$pdf_mediciones 					= $_POST['pdf_mediciones'];
$pdf_recetas 						= $_POST['pdf_recetas'];
$pdf_lista_de_compra 				= $_POST['pdf_lista_de_compra'];
...
//-> Nombre del archivo
$nombre_archivo = 'Mi Dieta | '.$cliente['nombre'].' '.$cliente['apellidos'].' Fecha: '.date('d-m-Y');
...
class MYPDF extends TCPDF {
    ...
    public function Header() {  		
		...
		if ($this->page == 1) {
			...
			if($_SESSION['img_alto'] <=140){
				$this->Cell(0, 10, $_SESSION['nombre_clinica'].' '.$_SESSION['img_alto'], 0, false, 'R', 0, '', 0, false, 'T', 'M');
			}else{	
				$this->Cell(0, 23, $_SESSION['nombre_clinica'].' '.$_SESSION['img_alto'], 0, false, 'R', 0, '', 0, false, 'T', 'M');
			}	
...
// define some HTML content with style		
	...
	$estados_y_necedidades .='<strong>Necesita en total '.$cliente['gasto_energetico'].' calorías para mantener su peso actual</strong></p>';
	...
	if(!empty($cliente['recomendaciones'])){
		$mostrar_recomendaciones ='';
		$mostrar_recomendaciones .= $html_header;	
		$mostrar_recomendaciones .='<body class="recomendaciones_y_mediciones">';			
			$mostrar_recomendaciones .='<h1 class="texto_primario text_center">Recomendaciones: </h1>';
			...
			$mostrar_recomendaciones .= $recomendaciones2;		
		$mostrar_recomendaciones .='</body>';
		$mostrar_recomendaciones .= '</html>';
	}
...
if(!empty($pdf_equivalencia_medidas_caceras )){
//->Medidas Caseras
$pdf->AddPage();
...
if(!empty($cliente['recomendaciones'])){
	$pdf->AddPage();
	$pdf->writeHTML($mostrar_recomendaciones, true, false, true, false, '');
}

if(!empty($pdf_mediciones )){
	//->recomendaciones_y_mediciones
	$pdf->AddPage();
...
for ($i = 1; $i <= $consulta_de_dieta['duracion']; $i++) {
	...
	if(!empty($pdf_recetas)){
		$mostrar = html_entity_decode($mostrar);
		$mostrar = preg_replace('/<p id="elaboracion">(.*?)<\/p>/', 'xx', $mostrar);	
		$mostrar = str_replace('xx<br>', '', $mostrar);
		$mostrar = strip_tags($mostrar, '<strong><h1><h2><br>');	
	}else{
		$mostrar = html_entity_decode($mostrar);		
		$mostrar = strip_tags($mostrar, '<strong><h1><h2><br>');	
	}
...
$para =  'vgonzalez@i-diet.es, artife@gmail.com';
...
$msg .= "--".$separator."--";
if (mail(utf8_decode($para), utf8_decode($asunto), utf8_decode($msg), $headers)){

...
$pagina = 'Lista de Alimentos Activos';
$migas = array('');
$migas_url = array('');
...
?>
	...
	#link_desactivar_alimentos, #link_duplicar_alimentos {
		display: none;
	}
	</style>
</head>
...
<body>
    <div id="wrapper">
					...
					<div class="ibox-content">
						<div class="row">							
							...
							<div class="col-sm-4 m-b-xs">
								<button style="display:none; " type="submit" class="btn btn-sm btn-primary" style="margin-top: 28px;"> Buscar!</button> 
							</div>						
						</div>
					</div>
				</div>
		...
		<!-- Fin buscador -->
        <div class="row">
		<form id="mover" action="<?php echo $url_app; ?>acciones-alimentos" method="post">		
                        ...
                        <div class="ibox-title">
                            <div class="ibox-tools">
								...
								<ul class="dropdown-menu dropdown-user">
									<li><a id="link_desactivar_alimentos"> <i class="fa fa-low-vision text-navy"></i> Desactivar Alimentos</a></li>
									<li><a id="link_duplicar_alimentos"> <i class="fa fa-files-o text-navy"></i> Duplicar Alimentos</a></li>
								</ul>
                            </div>
                        </div>
                        <div class="ibox-content">
	...
	<script>
        ...
        $(document).ready(function(){			
			...
			var table_buscar = $('#example').DataTable({ 
				...
				processing: true,
				erverSide: true,								
				ajax: "<?php echo $url_app; ?>crear-archivo-txt-alimentos.php",
				deferRender:    true,
				scrollCollapse: true,
				scroller:       false,
				serialize:		true,
		...
		$("#link_desactivar_alimentos" ).click(function() {
			$('#mover').attr('action', "<?php echo $url_app; ?>desactivar-alimentos");
			$("#accion").attr('value', 'Desactivado');
			$("#mover" ).submit();
		});
		$("#link_duplicar_alimentos" ).click(function() {
		...
		$('#example').on( 'click', 'tr', function () {
			...
			if(total_marcados == 0){
				$('#link_desactivar_alimentos').css('display', 'none');
				$('#link_duplicar_alimentos').css('display', 'none');
			}
			if(total_marcados >= 1){
				$('#link_desactivar_alimentos').css('display', 'block');
				$('#link_duplicar_alimentos').css('display', 'block');
			}

                        ...
                        <div class="ibox-title">
                            <div class="ibox-tools">
								...
								<ul class="dropdown-menu dropdown-user">
									<li><a id="vinculo_crear_plantilla" href="<?php echo $url_app; ?>nueva-regla"><i class="fa fa-low-vision text-navy"></i> Crear Plantilla</a></li>
									<li><a id="vinculo_desactivar_plantilla" href="#"> <i class="fa fa-low-vision text-navy"></i> Desactivar Plantillas</a></li>
								</ul>
                           </div> 
                        </div> */ ?>
	...
	<script>
        ...
        $(document).ready(function(){
			var table_buscar = $('#example').DataTable({  
				...
				processing: true,
				erverSide: true,							
				ajax: "<?php echo $url_app; ?>crear-archivo-txt-plantillas.php",
				deferRender:    true,
				scrollCollapse: true,
				scroller:       false,
				serialize:		true,
		...
		$(".marcar_todos").on("click", function () {
			...
			if(total_marcados == 0){
				$('#vinculo_desactivar_plantilla').css('display', 'none');
			}
			if(total_marcados >= 1){
				$('#vinculo_desactivar_plantilla').css('display', 'block');
			}
		});  
		$('#example').on( 'click', 'tr', function () {
			var total_marcados = $('.marcar').filter(':checked').length
		...
		$("#vinculo_desactivar_plantilla").on("click", function () {
            $('#formulario_reglas').attr('action', "<?php echo $url_app; ?>desactivar-plantillas");

...
for ($i = 0; $i <= count($ingredientes); $i++) {	
	if(isset($ingredientes[$i]['id_alimento'])){
		$datos_alimento[$i] = obtener_informacion_nutricional($ingredientes[$i]['id_alimento'], $ingredientes[$i]['cantidad']*$factor); 
		$todos_los_ingredientes_a_mostrar[$i]['id'] = $ingredientes[$i]['id_alimento'];
		$todos_los_ingredientes_a_mostrar[$i]['cantidad'] = $ingredientes[$i]['cantidad']*$factor;
		$i_continue++;
	}
}
...
<div class="row">
	<div class="col-lg-12">
		<h1><strong>Nombre:<br>	</strong> <?php echo utf8_encode($receta['nombre']); ?></h1><br>	
		<p style="text-align: left;"><?php if(!empty($receta['descripcion'])){  echo str_replace('\r\n','<br>', utf8_encode($receta['descripcion']));  } ?></p>										
	</div>
</div><br><br>	
<div class="row">
...
<div class="row">
	<div class="col-lg-12 text-left">
		...
		<tfoot>
			<tr>				
				<th></th>
				<th class="text-right"><?php echo $total_cantidad_mostrar; ?>g Total Cantidad</th>	
				<th class="text-right"><?php echo $mostrar_kcal_100g; ?> Kcal/100g</th>										
			</tr>
		</tfoot>
		</table>
	</div>	

...
foreach($_POST as $nombre => $valor)  {		
	...
	if (substr($nombre, 0, 7) == 'indice_' && !empty($valor)){				
		...
		//Listado temporal
		$lista_temporal[$t]['id_alimento'] = $id_alimento_nuevo;
		$lista_temporal[$t]['cantidad'] = $valor_alimento_nuevo;
		
		$t++;
	}   
}
if($lista_temporal != ''){
	$listado_completo_final = array_merge($lista_temporal, $datos_alimento);
}else{
	$listado_completo_final = $datos_alimento;
}
...
<body>
    <div id="wrapper">
					...
					<div class="row">
						<div class="col-md-8">
							<div class="ibox-content" id="datos_obligatorios">
								...
								<input type="text" class="form-control" name="nombre" id="nombre" placeholder="Título" required="" value="<?php echo utf8_encode($receta['nombre']); ?>	">								
								...
								<div class="ibox-content no-padding">
									...
									<div class="row" style="position: absolute; margin-top: 0px; top: 20px; right: 45px;">
										<div class="col-md-12">								
											...
											<a href="<?php echo $url_app; ?>lista-recetas" class="btn btn-w-m btn-primary">Atras</a>
											<?php if($receta['id_usuario'] == 0){ ?>
											<button id="duplicar_alimento_original" type="submit" class="btn btn-w-m btn-primary" title="La suma de Hidratos, Proteínas y Grasas debe dar 100">Duplicar</button>
											<?php } else{  ?>
											<button id="guardar_alimento" type="submit" class="btn btn-w-m btn-primary" title="La suma de Hidratos, Proteínas y Grasas debe dar 100">Guardar</button>
											<?php } ?>
											</div>
										</div>
									</div>
							...
							<div class="col-md-6">
								<div class="row" style="float: right;">
									<div class="col-md-12">								
										...
										<a id="boton_eliminar_alimento" class="btn btn-w-m btn-eliminar" disabled>Eliminar</a>
		...
		<!-- Fin buscador -->        
                ...
                <div class="footer">
					<?php if($receta['id_usuario'] != $_SESSION['id_usuario'] AND $receta['id_usuario'] != 0){ ?>
			...
			<table id="tabla_agregar_alimento" class="table table-striped dataTables-example select-filter">
				...
				<tbody>						
						...
						<?php for ($i = 0; $i <= count($listado_completo); $i++) { ?>						
							...
							<?php 
								//Buscamos en el array completo
								if(array_search($listado_completo[$i]['id_definitivo'], array_column($listado_completo_final, 'id_alimento')) !== False) {
									$en_indice = array_search($listado_completo[$i]['id_definitivo'], array_column($listado_completo_final, 'id_alimento'));
									$check_si = 'checked'; 
									$cantidad_mostrar = $listado_completo_final[$en_indice]['cantidad'];
								}else{
									$en_indice = '';
									$check_si = ''; 
									$cantidad_mostrar = 0; 
	...
	<script>
        ...
        $(document).ready(function(){
			...
			$("#boton_eliminar_alimento").click(function(){
				$("#example tbody").find('input[class="mostrar_ingrediente"]').each(function(){
					if($(this).is(":checked")){
						var nombre_temp = $(this).attr('id');
						var nombre_temp_2 = nombre_temp.slice(20); 					
						// todos_los_ids[] = nombre_temp_2;
						$(this).parents("tr").remove();								
						$("input[name = 'insert_alimento_"+nombre_temp_2+"']").attr('value', 0);	
						$("#valor_"+nombre_temp_2).attr('value', 0);	
						$("input[name = 'nuevo_ingrediente_"+nombre_temp_2+"']").prop('checked', false);	
					}				
				});
			});	
		});	
		...
		function pasar_a_temporal_variables () {	
				...
				// var temp_descripcion = $('.note-editable').html(encodedStr);								
				var temp_descripcion = $('.note-editable').html();								
				...
				if(temp_descripcion != ''){
					$("#temp_descripcion").attr('value', temp_descripcion);							
				}
				if(temp_peso_minimo != ''){
		...
		$("#duplicar_alimento_original").on("click", function () {
            $('#formulario_editar_receta').attr('action', "<?php echo $url_app; ?>duplicar-receta-original/");			
			$("#formulario_editar_receta" ).submit();
        });
		//Boton para editar receta
		$("#guardar_alimento").on("click", function () {
            $('#formulario_editar_receta').attr('action', "<?php echo $url_app; ?>update-receta/");			
			$("#formulario_editar_receta" ).submit();
        });
    </script>		
	<?php $conn->close(); ?>

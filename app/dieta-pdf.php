...
//-> Nombre del archivo
$nombre_archivo = 'Mi Dieta - '.salida_nombre($_SESSION['pdf']['datos_cliente']['nombre']).' '.salida_nombre($_SESSION['pdf']['datos_cliente']['apellidos']).' Fecha: '.date('d-m-Y');

$_SESSION['imagen_ruta'] = $url_app.'img/clinicas/'.$_SESSION['pdf']['datos_clinica']['logo'];
$_SESSION['nombre_clinica'] = $_SESSION['pdf']['datos_clinica']['nombre'];


$img_file = $_SESSION['imagen_ruta']; 	 // Dirección de la imagen 
$img_imagen = getimagesize($img_file);	//Sacamos la información
$img_ancho = $img_imagen[0];			//Ancho
$img_alto = $img_imagen[1];				//Alto
$_SESSION['img_alto'] = $img_alto;
  
  
$_SESSION['paciente'] = salida_nombre($_SESSION['pdf']['datos_cliente']['nombre']).', '.salida_nombre($_SESSION['pdf']['datos_cliente']['apellidos']);
...
class MYPDF extends TCPDF {
    ...
    public function Header() {  		
		...
		if ($this->page == 1) {
			...
			if($_SESSION['img_alto'] <=140){
				$this->Image($image_file, 10, 5, 15, 0, 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
			}else{
				$this->Image($image_file, 10, '', 15, 0, 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);	
			}
			// Set font
			$this->SetFont('helvetica', 'B', 8);
			if($_SESSION['img_alto'] <=140){
				$this->Cell(0, 10, salida_nombre($_SESSION['nombre_clinica']).' '.$_SESSION['img_alto'], 0, false, 'R', 0, '', 0, false, 'T', 'M');
			}else{	
				$this->Cell(0, 23, salida_nombre($_SESSION['nombre_clinica']).' '.$_SESSION['img_alto'], 0, false, 'R', 0, '', 0, false, 'T', 'M');
			}	
...
// ---------------------------------------------------------
		...
		$portada .= $html_header;
		$portada .= '<body>';			
			$portada .= '<p class="text_center"><img src="'.$url_app.'img/clinicas/'.$_SESSION['pdf']['datos_clinica']['logo'].'" style="width:500px"></p>';
			$portada .= '<h1 class="text_center">'.salida_nombre($_SESSION['pdf']['datos_clinica']['nombre']).'</h1>';
			$portada .= '<h2 class="text_center">'.salida_nombre($_SESSION['pdf']['datos_clinica']['direccion']).' - '.$_SESSION['pdf']['datos_clinica']['localidad'].'</h2>';			
			$portada .= '<h3 class="text_center">'.$_SESSION['pdf']['datos_clinica']['telefono'].'</h3>';			
			$portada .= '<h4 class="text_center texto_primario" style="font-size:30px;">ESTUDIO <br> PERSONALIZADO</h1>';
			$portada .= '<h1 class="text_center texto_primario">'.$_SESSION['pdf']['datos_cliente']['nombre'].' '.$_SESSION['pdf']['datos_cliente']['apellidos'].'</h1>';							
		$portada .= '</body>';
		$portada .= '</html>';	
		...
		if($_SESSION['pdf']['datos_cliente']['sexo'] == 'Hombre'){
			$introduccion .= '<h1 class="texto_primario">BIENVENIDO</h1>';
			$introduccion .= '<p><strong>Estimado '.salida_nombre($_SESSION['pdf']['datos_cliente']['nombre']).' </strong></p>';
		}else{
			$introduccion .= '<h1 class="texto_primario">BIENVENIDA</h1>';
			$introduccion .= '<p><strong>Estimada '.salida_nombre($_SESSION['pdf']['datos_cliente']['nombre']).' </strong></p>';
		}		
		...
		$estados_y_necedidades .='<strong>Nombre del paciente: </strong>'.salida_nombre($_SESSION['pdf']['datos_cliente']['nombre']).' '.$_SESSION['pdf']['datos_cliente']['apellidos'].'<br>';
		$estados_y_necedidades .='<strong>Fecha de nacimiento: </strong>'.$_SESSION['pdf']['datos_cliente']['fecha_nacimiento'].'<br>';
		$estados_y_necedidades .='<strong>Peso: </strong>'.$_SESSION['pdf']['ultimo_peso_cliente']['peso'].'Kg'.'<br>';
		$estados_y_necedidades .='<strong>Altura: </strong>'.$_SESSION['pdf']['datos_cliente']['altura'].'cm'.'<br>';
		$estados_y_necedidades .='<strong>Actividad física: </strong>'.$_SESSION['pdf']['ultimo_peso_cliente']['informacion'].'<br>';
		$estados_y_necedidades .='<strong>Índice de masa corporal: </strong>'.$_SESSION['pdf']['ultimo_peso_cliente']['inice_masa_corporal'].'<br>';
		
		
	if(count($_SESSION['pdf']['historial_pesos_grafico']) >= 2){ 
		$estados_y_necedidades .='<h2>Historial de pesos</h2>';
		$estados_y_necedidades .='<img src="'.$url_app.'pdf-completo/generar-grafico-de-barras.php?id='.$_SESSION['pdf']['id_cliente'].'" />';	
		$estados_y_necedidades .= $_SESSION['pdf']['id_cliente'];
		$estados_y_necedidades .='<p></p><p></p>';
	 } 
	$estados_y_necedidades .='<h2>Necesidades energéticas</h2>';
	$estados_y_necedidades .='<strong>Metabolismo basal: </strong>'.$_SESSION['pdf']['ultimo_peso_cliente']['metabolisto_basal'].'<br />';
	$estados_y_necedidades .='<strong>Consumo por actividad física: </strong>'.($_SESSION['pdf']['ultimo_peso_cliente']['gasto_energetico_total'] - $_SESSION['pdf']['ultimo_peso_cliente']['metabolisto_basal']).' calorías diarias<br />';
	$estados_y_necedidades .='<strong>Gasto energético actual: </strong>'.$_SESSION['pdf']['ultimo_peso_cliente']['gasto_energetico_total'].' calorías </p>';
	...
	if(count($_SESSION['pdf']['grupos_excludios']) >= 1){
		for ($i = 0; $i <= $_SESSION['pdf']['total_grupos']; $i++) {
			if(!empty($grupos_alimentos[$i]['id_grupo'])){
				if (in_array($grupos_alimentos[$i]['id_grupo'], $_SESSION['pdf']['grupos_excludios'])) {
					$estados_y_necedidades .= salida_nombre($grupos_alimentos[$i]['grupo']).', ';
				}
			}
		}
	}  	
	...
	$recomendaciones_y_mediciones .='<body class="recomendaciones_y_mediciones">';	
		if(!empty($_SESSION['pdf']['datos_cliente']['recomendaciones'])){
			$recomendaciones_y_mediciones .='<h1 class="texto_primario text_center">Recomendaciones: </h1>';
			$html2 = $_SESSION['pdf']['datos_cliente']['recomendaciones'];
			$entities_correction2= html_entity_decode( $html2, ENT_COMPAT, 'UTF-8');
			$recomendaciones2 = html_entity_decode( $entities_correction2, ENT_COMPAT, 'UTF-8');		
			$recomendaciones_y_mediciones .= $recomendaciones2;		
		}							
...
for ($i = 1; $i <= $total_semanas_sliders; $i++) {
	...
	$lista_horizontal_dieta .= $html_header;	
	$lista_horizontal_dieta .='<body class="recomendaciones_y_mediciones">';		
	$lista_horizontal_dieta .='<h1>Semana '.$i.' de '.$total_semanas_sliders.'</h1>';

...
if($datos_alimento['id_usuario'] == 0){
	$disable = 'disabled';
}else{
	$disable = '';
}

//Si el alimento no pertenece a este usuario sacar de la edicion
if($datos_alimento['id_usuario'] != $_SESSION['id_usuario'] AND $datos_alimento['id_usuario'] != 0){
	$_SESSION['mensaje'] = 'alimento_de_otro_usuario';
...
<body>
    <div id="wrapper">
		...
		<!-- Buscador -->
		<?php if($datos_alimento['id_usuario'] == 0){ ?>
			<form action="<?php echo $url_app; ?>duplicar-alimento-original" method="post">						
		<?php }else{ ?>
			<form action="<?php echo $url_app; ?>actualizar-alimento" method="post">						
		<?php } ?>
		<div class="row">
            <div class="col-lg-12">
					...
					<div class="ibox-content">
						...
						<div class="row">
							<div class="col-md-2">
								...
								<input name="id_usuario" type="hidden" value="<?php echo $datos_alimento['id_usuario'];?>" required>
							</div>
							<div class="col-md-8">
								<div class="row">
									<div class="col-md-1"> Nombre  </div>
									<div class="col-md-5"><input type="text" placeholder="Nombre" class="input-sm form-control" name="nombre" value="<?php echo utf8_encode($datos_alimento['nombre']);?>" <?php echo $disable; ?> required></div>
									<div class="col-md-1"> Hidratos  </div>
									<div class="col-md-5 sumar_datos">									
										<input id="hidratos_porc" name="hidratos_porc" placeholder="Hidratos" class="touchspin1" type="text" value="<?php echo $datos_alimento['hidratos_porc'];?>" <?php echo $disable; ?> required>
									</div>
								</div>
								<br />
								<div class="row">
									<div class="col-md-1"> Kcal/100g  </div>
									<div class="col-md-5"><input type="number" placeholder="Kcal/100g" class="input-sm form-control" name="kcal_100g" value="<?php echo $datos_alimento['kcal_100g'];?>" <?php echo $disable; ?> required></div>
									<div class="col-md-1"> Proteínas  </div>
									<div class="col-md-5 sumar_datos"><input id="proteinas_porc" type="number" placeholder="Proteínas" class="touchspin2" name="proteinas_porc" value="<?php echo $datos_alimento['proteinas_porc'];?>" min="0" max="100"  <?php echo $disable; ?> required></div>
								</div>
								<br />
								<div class="row">
									<div class="col-md-1"> Grupo  </div>
									<div class="col-md-5">
									<select class="input-sm form-control input-s-sm" style="padding-top:0px;" name="grupo" <?php echo $disable; ?>>										
									<?php echo grupo_de_alimentos_options_select($datos_alimento['grupo']); ?>																			
									</select>
									</div>
									<div class="col-md-1"> Grasas:  </div>
									<div class="col-md-5 sumar_datos"><input id="grasas" type="number" placeholder="Grasa" class="touchspin3" name="grasa_porc" value="<?php echo $datos_alimento['grasa_porc'];?>" min="0" max="100" <?php echo $disable; ?> required></div>
								</div>
							</div>
							<div class="col-md-2">
							</div>
						...
						<div class="row">
							<div class="col-md-12">								
								...
								<?php if($datos_alimento['id_usuario'] == 0){ ?>
								<button id="guardar_alimento" type="submit" class="btn btn-w-m btn-primary" title="La suma de Hidratos, Proteínas y Grasas debe dar 100" disabled>Duplicar</button>
								<?php }else{ ?>
					...
					<div class="ibox-content">
						...
						<div class="row">
							<div class="col-md-12">								
								...
								<a href="<?php echo $url_app; ?>lista-alimentos" class="btn btn-w-m btn-atras">Atras</a>
								<?php if($datos_alimento['id_usuario'] == 0){ ?>
								<button id="guardar_alimento2" type="submit" class="btn btn-w-m btn-primary" title="La suma de Hidratos, Proteínas y Grasas debe dar 100" disabled>Duplicar</button>
								<?php }else{ ?>

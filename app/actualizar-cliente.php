...
if (!empty($_POST["dni_cliente"])){	
	...
	$imc 				= $_POST["imc"];
	$edad 				= $_POST["edad"];
	$informacion 		= entrada_nombre($_POST["informacion"]);
	...
	actualizar_cliente ($id_cliente, $dni, $nombre, $apellidos, $sexo, $telefono_fijo, $telefono_movil, $direccion, $cp, $localidad, $peso, $edad, $altura, $fecha_nacimiento, $email, $comentarios, $recomendaciones, $nombre_completo, $actividad, $fecha_de_alta);
	
	//Obtener el ultimo listado de peso y si es igual no duplicarlo
	...
	if(!empty($historial_pesos)){
		...
		if($Ultimo_registro_peso['metabolisto_basal'] != $metabolismo || $Ultimo_registro_peso['gasto_energetico_total'] != $gasto || $Ultimo_registro_peso['inice_masa_corporal'] != $imc){
			$nuevo_registro = 'si';
		...
		//Agregar al historial en caso de que los borrara todos
		nuevo_registro_pesox_cliente ($id_cliente, $fecha_de_alta, $peso, $edad, $informacion, $metabolismo, $gasto, $imc);
	}
	...
	eliminar_registro_grupos_alimentos_x_clientes ($id_cliente);
	eliminar_registros_alimentos_x_clientes ($id_cliente);
	...
	$_SESSION['mensaje'] = 'cliente_actualizado';		
	header('location:'.vinculo_completo('lista-clientes'));	

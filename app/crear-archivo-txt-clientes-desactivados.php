...
include_once 'parts/configuracion.php';
	...
	$query = "SELECT * FROM `gx_clientes` WHERE id_usuario='".$_SESSION['id_usuario']."' AND status = '2'";	
	...
	while($row = $result->fetch_assoc()) {		
		...
		if(!empty($mostrar[$i]['id_cliente'])){			
			...
			//Editar Alimento
			$dni = "<a href='".$url_app."editar-cliente/".$mostrar[$i]['id_cliente']."'>".salida_nombre($mostrar[$i]['dni'])."</a>";
			
			//Contacto
			$contacto = 
			
			$html .='[ "'.$input.'", "'.$dni.'", "'.salida_nombre($mostrar[$i]['nombre']).'", "'.$mostrar[$i]['apellidos'].'", "'.$mostrar[$i]['contacto'].'" ],';	
			$input = "";
		}	
		$i++;
	}	
